...
```{r}
# Join LSTM and DNN results
rolling_r2_sites <- lstm_lso |>
  left_join(
    dnn_lso,
    by = c("sitename", "date", "GPP_NT_VUT_REF")
  ) |>
  group_by(sitename) %>%
  reframe(r2_lstm = rolling_rsq(., truth = 'GPP_NT_VUT_REF', estimate = "gpp_lstm"),
          r2_dnn = rolling_rsq(., truth = 'GPP_NT_VUT_REF', estimate = 'gpp_dnn'))
```
